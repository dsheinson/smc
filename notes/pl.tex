...
and $s_{x,t} = (m_t,C_t)$ are the sufficient statistics for $x_t|\theta$ that can be updated by the recursive equations
\begin{align*}
...
\end{align*}
where $A_t = \frac{F_t^2(\phi^2C_{t-1} + \sigma^2_s)}{F_t^2(\phi^2C_{t-1} + \sigma^2_s) + \sigma^2_m}$. Lastly, we derive the conditional distributions for $\theta$ using \[p(\theta|y_{1:t},x_{0:t}) = \]
