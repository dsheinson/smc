...
# Function to run pfs
fmri_rm <- function(N, mod.sim, dimx, n, n.sim, n.run, mod.est, np, sd.fac = 1, alpha = 0.05, progress = TRUE, save = FALSE)
{
   # Load simulated data
  ...
  dllik <- function(y, x, theta) get(paste("dllik.",mod.est,sep=""))(y, x, theta, mysim$true.params$U[1,2,])
  rmove <- function(y, x, theta) rmcmc(y, x, theta, mysim$true.params$U[1,2,], mod.est, prior)
  out = rm_pf(mysim$y, dllik, revo, rprior, rmove, np, store.all = save, progress=progress, method="stratified", nonuniformity="ess", threshold=0.8, log=FALSE)
  ...
  if(save){
    out.state = array(NA, dim=c(2,np,nt+1))
    for(i in 1:(nt+1)) out.state[,,i] = out$state[[i]][,,i]
  } else { out.state = out$state}
  ...
  if(!save) pf.out = list(lmarglik=lmarglik, state.quant=state.quant, theta.quant=theta.quant) else pf.out = list(out=out, prior=prior, wt.mom=wt.mom, lmarglik=lmarglik, state.quant=state.quant, theta.quant=theta.quant)
...
require(doMC)
registerDoMC()
# data1 = expand.grid(N = 20, mod.est = c("M101","M011"), dimx = 2, n.sim = 1:8, n.run = 1:3, mod.sim = c("M101","M011"), sd.fac = 1, n = c(1,6,11,16), np = 100, progress = FALSE)
# data2 = expand.grid(N = 20, mod.est = c("M101","M011"), dimx = 2, n.sim = 1:8, n.run = 1:3, mod.sim = c("M101","M011"), sd.fac = c(3,5,7), n = 6, np = 100, progress = FALSE)
# data1 = expand.grid(N = 20, mod.est = "M011", dimx = 2, n.sim = c(1,3), n.run = 2:3, mod.sim = "M011", sd.fac = 1, n = 16, np = 100, progress = FALSE, save = FALSE, stringsAsFactors = FALSE)
# data2 = expand.grid(N = 20, mod.est = "M011", dimx = 2, n.sim = c(5,7), n.run = 1:3, mod.sim = "M011", sd.fac = 1, n = 16, np = 100, progress = FALSE, save = FALSE, stringsAsFactors = FALSE)
# data3 = expand.grid(N = 20, mod.est = "M011", dimx = 2, n.sim = c(1,3,5,7), n.run = 1:3, mod.sim = c("M101","M011"), sd.fac = c(5,7), n = 6, np = 100, progress = FALSE, save = FALSE, stringsAsFactors = FALSE)
mydata = expand.grid(N = 20, mod.est = c("M101","M011"), dimx = 2, n.sim = 1:8, n.run = 1:3, mod.sim = c("M101","M011"), sd.fac = 1, n = 6, np = 100, progress = FALSE, save = TRUE, stringsAsFactors = FALSE)
