...
nsims = 20
cols = c(3,4,6,7)
data1 = expand.grid(n.sim=rep(1,20),np=np, W=c(0.5,1,2), stringsAsFactors=FALSE)
data1 = data.frame(data1, seed = 1:dim(data1)[1])
data2 = expand.grid(n.sim=2:20,np=np, W=c(0.5,1,2), stringsAsFactors=FALSE)
...
cv_pf_quantiles <- function(n.sim, W = 1, alpha = 0.05, burn.k = 1, burn.p = 5)
{
  ...
  {
    seeds = mydata$seed[which(mydata$n.sim == n.sim & mydata$np == np[j] & mydata$W == W)]
    for(i in 1:length(seeds))
    {
      load(paste(dpath,"cv_pf-",n.sim,"-",np[j],"-",W,"-",seeds[i],"-",alpha,".rdata",sep=""))
  ...
  pdf(paste(gpath,"cv-pf-states-",n.sim,".pdf",sep=""))
  for(j in 1:length(np))
  {
    seeds = mydata$seed[which(mydata$n.sim == n.sim & mydata$np == np[j] & mydata$W == W)]
    for(i in 1:length(seeds))
    {
      load(paste(dpath,"cv_pf-",n.sim,"-",np[j],"-",W,"-",seeds[i],"-",alpha,".rdata",sep=""))
      if(j == 1 & i == 1) plot(0:nt,mysims[[n.sim]]$x[1,],ylim=c(gmin.k,gmax.k),type="l",xlab=expression(t),ylab="Position")
  ...
  legend("bottomright", c(paste(np," particles",sep=""),"True post", "True sim"), lty=rep(1,length(np)+2), col = c(cols,2,1))
  dev.off()
  
  # Plot 95% CI of filtered precision
  pdf(paste(gpath,"cv-pf-precision-",n.sim,".pdf",sep=""))
  for(j in 1:length(np))
  {
    seeds = mydata$seed[which(mydata$n.sim == n.sim & mydata$np == np[j] & mydata$W == W)]
    for(i in 1:length(seeds))
    {
      load(paste(dpath,"cv_pf-",n.sim,"-",np[j],"-",W,"-",seeds[i],"-",alpha,".rdata",sep=""))
      if(j == 1 & i == 1) plot(0:nt,rep(1,nt+1),ylim=c(gmin.p,gmax.p),type="l",xlab=expression(t),ylab=expression(sigma^2))
  ...
  title(expression(paste("Sequential 95% CI for ",sep=""),sigma^2))
  legend("topright", c(paste(np," particles",sep=""),"True post", "True sim"), lty=rep(1,length(np)+2), col = c(cols,2,1))
  dev.off()
}
...
rm.lmarglik = array(NA, c(nsims,length(np), length(W)))
for(j in 1:length(np))
{
  for(k in 1:length(W))
  {
    seeds = mydata$seed[which(mydata$n.sim == n.sim & mydata$np == np[j] & mydata$W == W[k])]
    for(i in 1:length(seeds))
    {
      load(paste(dpath,"cv_pf-",n.sim,"-",np[j],"-",W[k],"-",seeds[i],"-",alpha,".rdata",sep=""))
...
pdf(file=paste(gpath,"cv_pf_loglik.pdf",sep=""),width=15,height=5)
...
pdf(file=paste(gpath,"cv_pf_ternary.pdf",sep=""),width=10,height=10)
par(mfrow=c(2,2))
...
true.lmarglik = maply(expand.grid(n.sim=1:nsims, W=W), true_lmarglik)
...
for(j in 1:length(np))
{
  for(k in 1:length(W))
  {
    for(i in 1:nsims)
    {
      seed = mydata$seed[which(mydata$n.sim == i & mydata$np == np[j] & mydata$W == W[k])][1]
      load(paste(dpath,"cv_pf-",i,"-",np[j],"-",W[k],"-",seed,"-",alpha,".rdata",sep=""))
      rm.lmarglik[i,j,k] = pf.out$lmarglik
    }
  }
}
