...
fmri_rm <- function(N, mod.sim, dimx, n, n.sim, n.run, mod.est, np, sd.fac = 1, alpha = 0.05, progress = TRUE, save = FALSE)
{
  ...
  if(!save) pf.out = list(lmarglik=lmarglik, state.quant=state.quant, theta.quant=theta.quant) else pf.out = list(out=out, lmarglik=lmarglik, state.quant=state.quant, theta.quant=theta.quant)
...
data4 = expand.grid(N = 20, mod.est = c("M101","M011"), dimx = 2, n.sim = 1:8, n.run = 1:3, mod.sim = c("M101","M011"), sd.fac = 1, n = 6, np = 100, progress = FALSE, save = TRUE, stringsAsFactors = FALSE)
